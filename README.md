...
Then seed the published levels (the level seed is generated by
`npx tsx scripts/generate-level-seed.ts`):

```bash
npm run db:seed
```

For a fresh local database, `npm run db:setup` runs migrations first and seeds second.
