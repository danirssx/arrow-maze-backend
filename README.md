...
migration from schema changes. Published-level seed data is generated by
`npm run seed:generate`, which (re)writes `prisma/seed-data/levels.ts`; the seed
itself is idempotent (every write is an upsert).
