 ...
 *
 * Run with: `npx tsx scripts/generate-level-seed.ts`.
 */
...
function sqlJsonb(records: readonly ArrowRecord[]): string {
...
LEVELS.forEach((meta, index) => {
  ...
  rows.push(
    `-- Level ${index + 1}: ${meta.name} (${meta.difficulty}, ${meta.arrowCount} arrows, ` +
      `${generated.occupiedCells}/${meta.rows * meta.cols} cells` +
