...
function makeEmptyProgress(): PlayerProgress {
  return PlayerProgress.empty(new ProgressId('progress-1'), new UserId('user-1'));
}
...
describe('PgProgressRepository', () => {
  describe('findByUserId', () => {
    it('should_return_null_when_no_progress_found', async () => {
      ...
      // Act
      const result = await repo.findByUserId(new UserId('user-1'));
    ...
    it('should_rehydrate_progress_with_completed_levels_when_found', async () => {
      // Arrange
      const progressRow = { id: 'progress-1', user_id: 'user-1', version: 2, updated_at: new Date() };
      const levelRow = {
        id: 'cl-1', level_id: 'level-1', best_score: 200, best_time_seconds: 25,
      ...
      expect(result!.completedLevels[0].levelId.value).toBe('level-1');
    ...
    it('should_throw_infrastructure_error_when_query_fails', async () => {
      ...
      // Act & Assert
      await expect(repo.findByUserId(new UserId('user-1'))).rejects.toThrow(InfrastructureError);
    });
  });
