 ...
 *
 * Extracted from `seed.ts` (MAZ-194) so it can be validated by tests without executing
 * the seed. It reproduces the former `002_seed_demo_data.sql` demo state: `demo_player`
 * has completed the first three levels and the first three levels have leaderboards.
 *
 * The QA full-catalog account (`QA_FULL_CATALOG_USER_ID`) is intentionally absent here:
